...
//        bloodSugarList.add(0, Html.fromHtml(myString));
//        adapter.notifyDataSetChanged();
    }

    public ArrayList<Spanned> getInfoFromDB() {
        ArrayList<Spanned> fromDB = new ArrayList<>();
        Cursor cursor = myDB.getAllSugarData();
        if (cursor.moveToFirst()) {
            do {
                if (cursor.getString(1).equals(username)) {
                    ...
                    fromDB.add(0, Html.fromHtml(formatInput(bloodSugarInput, foodInput, dateInput)));
                }
